...
| File                   | Purpose                                              |
|------------------------|------------------------------------------------------|
| `pso_aco_planner.cpp`  | Main C++ source (PSO + ACO + dynamic + combined opt) |
...
| `sd_pairs.csv`         | Source-destination pairs to process                  |
...
| `COMBINED_OPT` | 1       | 0 = independent only, 1 = run combined opt    |
| `COMBINED_ITERS`| 5      | Max combined optimization iterations          |
| `COMBINED_WEIGHT`| 0.3   | Traffic accumulation weight per iteration      |
| `STABILITY_THRESH`| 0.05 | Stop if avg cost change < this fraction       |
...
### Combined Route Optimization (Step B)

When `COMBINED_OPT = 1`, after independent optimization the program runs
an iterative combined optimization:

1. Runs all S-D pairs (same flowchart as Step A)
2. Identifies shared edges (used by 2+ routes)
3. Increases traffic on shared edges proportional to usage
4. Re-runs all pairs with updated traffic
5. Repeats until costs stabilize (change < `STABILITY_THRESH`)

Output files produced:
- `combined_pairs_results.csv` — final combined route results
- `network_analysis_combined.txt` — network analysis with combined routes
- `combined_comparison.txt` — side-by-side comparison report
