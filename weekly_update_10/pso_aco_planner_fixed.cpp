 ...
 *  Uses FIXED (alpha, beta, rho) -- NO PSO optimization step.
 *  Implements the "latest_flowchart.png" approach.
 *
 ...
 *  Flowchart : latest_flowchart.png (Fixed alpha, beta, rho)
 *
...
struct Config {
    ...
    // Fixed ACO parameters (used instead of PSO-tuned values)
...
//  (Fixed alpha, beta, rho -- NO PSO step, per latest_flowchart.png)
// =============================================================================
    ...
    int pair_index)
{
    ...
    // Use FIXED parameters (no PSO optimization)
    double use_alpha = cfg.fixed_alpha;
    double use_beta  = cfg.fixed_beta;
    double use_rho   = cfg.fixed_rho;

    result.pso_alpha = use_alpha;
    result.pso_beta  = use_beta;
    result.pso_rho   = use_rho;

    std::cout << "\n  [FIXED] Using fixed ACO parameters:\n"
              << "        alpha = " << std::fixed << std::setprecision(4) << use_alpha << "\n"
              << "        beta  = " << use_beta  << "\n"
              << "        rho   = " << use_rho   << "\n";

    // =========================================================================
    //  PHASE 1 -- Initial ACO path (besttour) with fixed (alpha, beta, rho)
    //  (Flowchart: "PHASE 1 -- Initial Pathfinding")
    // =========================================================================
    std::cout << "\n  --- PHASE 1: Initial ACO Path Planning (Fixed Params) ---\n";
    ...
    std::cout << "\n  [ACO] Initial best path found (FIXED params):\n";
    print_path(initial.path, initial.cost, "besttour(fixed)");

    // ---- Also run PSO-tuned ACO for comparison (same edge state) ----
    std::cout << "\n  --- [COMPARISON] Running PSO to tune params on SAME traffic state ---\n";
    PSOResult pso_compare = run_pso(edges, adj, n, src, dst, cfg);
    ACOResult pso_aco_compare = run_aco(
        edges, adj, n, src, dst,
        pso_compare.alpha, pso_compare.beta, pso_compare.rho,
        cfg.Q_const, cfg.tau_init,
        cfg.num_ants, cfg.aco_iter,
        cfg.avg_vehicle_len);

    if (pso_aco_compare.found) {
        std::cout << "  [ACO] PSO-tuned path found:\n";
        print_path(pso_aco_compare.path, pso_aco_compare.cost, "besttour(PSO)");
        std::cout << "  [CMP] Fixed initial cost = " << std::fixed << std::setprecision(2)
                  << initial.cost << "  |  PSO initial cost = " << pso_aco_compare.cost;
        if (pso_aco_compare.cost < initial.cost) {
            std::cout << "  --> PSO is BETTER by "
                      << std::setprecision(1)
                      << ((initial.cost - pso_aco_compare.cost) / initial.cost * 100) << "%\n";
        } else {
            std::cout << "  --> Fixed is better or equal\n";
        }
        // Store PSO initial cost for later comparison
        result.pso_alpha = pso_compare.alpha;  // Override with actual PSO values for report
        result.pso_beta  = pso_compare.beta;
        result.pso_rho   = pso_compare.rho;
    } else {
        std::cout << "  [CMP] PSO-tuned ACO could not find path.\n";
    }
    // Store comparison data in a static vector for the final report
    static std::vector<std::pair<double,double>> pso_vs_fixed_initial_costs;
    pso_vs_fixed_initial_costs.push_back({
        pso_aco_compare.found ? pso_aco_compare.cost : -1.0,
        initial.cost
    });

    // =========================================================================
    //  PHASE 2 -- Dynamic Re-planning Loop (continues with FIXED params)
    //  (Flowchart: "Dynamic monitoring" -> "Trigger condition?" -> etc.)
    // =========================================================================
    std::cout << "\n  --- PHASE 2: Dynamic Simulation & Re-planning ---\n";
...
int main() {
    print_banner("Fixed (alpha,beta,rho) Dynamic Path Planner  (Wu et al. 2020)");
    std::cout << "  Architecture: Fixed (alpha, beta, rho) -- NO PSO step\n"
              << "  Flowchart  : latest_flowchart.png\n"
              << "  Extended   : ALL S-D Pairs + Combined Route Optimization\n";
    ...
    std::cout << "\n[CFG] map=" << cfg.map_file
              ...
              << "\n  Fixed params: alpha=" << cfg.fixed_alpha
              << "  beta=" << cfg.fixed_beta
              << "  rho=" << cfg.fixed_rho << "\n";
    ...
    // =========================================================================
    //  STEP A: Independent optimization (FIXED alpha, beta, rho)
    //  Each pair runs: ACO with fixed params -> Dynamic Re-planning
    // =========================================================================
    print_banner("STEP A: INDEPENDENT OPTIMIZATION (Fixed alpha,beta,rho)");

    std::ofstream reroute_log("fixed_reroute_log.txt");
    ...
    for (int i = 0; i < (int)sd_pairs.size(); ++i) {
        ...
        std::cout << "\n  --- PAIR " << i << " FINAL REPORT ---\n"
                  ...
                  << "  Fixed params  : alpha=" << std::setprecision(3) << result.pso_alpha
    ...
    // -- Independent summary --
    print_banner("INDEPENDENT OPTIMIZATION SUMMARY (Fixed Params)");
    print_summary_table(independent_results, "Fixed-Independent");

    save_results(independent_results, "fixed_all_pairs_results.csv");
    analyze_network(independent_results, original_edges, n, "fixed_network_analysis.txt");
    ...
    if (cfg.combined_opt) {
        print_banner("STEP B: COMBINED ROUTE OPTIMIZATION (Fixed Params)");
        ...
        // -- Combined summary --
        print_banner("COMBINED OPTIMIZATION SUMMARY (Fixed Params, Final Iter)");
        print_summary_table(combined_results, "Fixed-Combined");

        save_results(combined_results, "fixed_combined_pairs_results.csv");
        analyze_network(combined_results, original_edges, n, "fixed_network_analysis_combined.txt");

        // -- Comparison report --
        analyze_stabilization(
            independent_results, combined_results,
            combined_history, "fixed_combined_comparison.txt");
    }

    // =========================================================================
    //  STEP C: Load PSO-optimized results and compare
    // =========================================================================
    print_banner("PSO-OPTIMIZED vs FIXED PARAMETER COMPARISON");

    // Try to load PSO results from all_pairs_results.csv
    std::ifstream pso_csv("all_pairs_results.csv");
    if (!pso_csv.is_open()) {
        std::cout << "\n  [INFO] all_pairs_results.csv not found.\n"
                  << "  Run pso_aco_planner.exe first to generate PSO-optimized results.\n"
                  << "  Then re-run this program to see the comparison.\n";
    } else {
        // Parse PSO results CSV
        struct PSORow {
            int pair_id, source, destination;
            ...
            int reroutes;
            std::string reached;
            double pso_a, pso_b, pso_r;
        };
        std::vector<PSORow> pso_rows;
        std::string csv_line;
        std::getline(pso_csv, csv_line); // skip header
        while (std::getline(pso_csv, csv_line)) {
            csv_line = trim(csv_line);
            if (csv_line.empty()) continue;
            std::stringstream ss(csv_line);
            std::string tok;
            PSORow row;
            std::getline(ss, tok, ','); row.pair_id = std::stoi(tok);
            std::getline(ss, tok, ','); row.source = std::stoi(tok);
            std::getline(ss, tok, ','); row.destination = std::stoi(tok);
            std::getline(ss, tok, ','); row.init_path = tok;
            std::getline(ss, tok, ','); row.init_cost = std::stod(tok);
            std::getline(ss, tok, ','); row.final_path = tok;
            std::getline(ss, tok, ','); row.final_cost = std::stod(tok);
            std::getline(ss, tok, ','); row.reroutes = std::stoi(tok);
            std::getline(ss, tok, ','); row.reached = tok;
            std::getline(ss, tok, ','); row.pso_a = std::stod(tok);
            std::getline(ss, tok, ','); row.pso_b = std::stod(tok);
            std::getline(ss, tok, ','); row.pso_r = std::stod(tok);
            pso_rows.push_back(row);
        }
        pso_csv.close();

        // Print comparison table
        std::cout << "\n  " << std::string(100, '=') << "\n";
        std::cout << "  PSO-OPTIMIZED (old_flowchart) vs FIXED (latest_flowchart) COMPARISON\n";
        std::cout << "  " << std::string(100, '=') << "\n\n";

        std::cout << "  " << std::setw(5) << "Pair"
                  << std::setw(5) << "Src"
                  << std::setw(5) << "Dst"
                  << std::setw(13) << "PSO Cost"
                  << std::setw(13) << "Fixed Cost"
                  << std::setw(12) << "Diff %"
                  << std::setw(9) << "Winner"
                  << "  PSO(a,b,r) vs Fixed(a,b,r)\n";
        ...
        int pso_reroutes_total = 0, fixed_reroutes_total = 0;

        int num_compare = std::min((int)pso_rows.size(), (int)independent_results.size());
        for (int i = 0; i < num_compare; ++i) {
            const auto& pr = pso_rows[i];
            const auto& fr = independent_results[i];

            std::string winner = "---";
            if (pr.reached == "YES" && fr.reached) {
                compared++;
                total_pso   += pr.final_cost;
                total_fixed += fr.final_cost;
                pso_reroutes_total += pr.reroutes;
                fixed_reroutes_total += fr.replan_count;

                if (pr.final_cost < fr.final_cost - 0.01) {
                    winner = "PSO";
                    pso_wins++;
                } else if (fr.final_cost < pr.final_cost - 0.01) {
                    winner = "FIXED";
                    fixed_wins++;
                } else {
                    winner = "TIE";
            ...
            double diff_pct = 0;
            if (pr.reached == "YES" && pr.final_cost > 0)
                diff_pct = (fr.final_cost - pr.final_cost) / pr.final_cost * 100;

            std::cout << "  " << std::setw(5) << i
                      << std::setw(5) << pr.source
                      << std::setw(5) << pr.destination
                      << std::setw(13) << std::fixed << std::setprecision(2) << pr.final_cost
                      << std::setw(13) << (fr.reached ? fr.final_cost : 0.0)
                      << std::setw(11) << std::setprecision(1) << diff_pct << "%"
                      << std::setw(9)  << winner
                      << "  (" << std::setprecision(2) << pr.pso_a << "," << pr.pso_b << "," << pr.pso_r << ")"
                      << " vs (" << cfg.fixed_alpha << "," << cfg.fixed_beta << "," << cfg.fixed_rho << ")"
                      << "\n";
        }

        std::cout << "\n  " << std::string(95, '=') << "\n";
        std::cout << "\n  OVERALL COMPARISON SUMMARY:\n\n";
        double avg_pso   = (compared > 0) ? total_pso / compared : 0;
        double avg_fixed = (compared > 0) ? total_fixed / compared : 0;
        double improvement = (avg_fixed > 0) ? (avg_fixed - avg_pso) / avg_fixed * 100 : 0;

        std::cout << "    Total pairs compared          : " << compared << "\n"
                  << "    PSO-Optimized avg final cost   : " << std::fixed << std::setprecision(2) << avg_pso << "\n"
                  << "    Fixed-param avg final cost     : " << avg_fixed << "\n"
                  << "    PSO improvement over Fixed     : " << std::setprecision(1) << improvement << "%\n\n"
                  << "    PSO wins (lower cost)          : " << pso_wins << "\n"
                  << "    Fixed wins (lower cost)        : " << fixed_wins << "\n"
                  << "    Ties                           : " << ties << "\n\n"
                  << "    PSO total reroutes             : " << pso_reroutes_total << "\n"
                  << "    Fixed total reroutes           : " << fixed_reroutes_total << "\n\n";

        if (pso_wins > fixed_wins && avg_pso < avg_fixed) {
            std::cout << "  ** CONCLUSION: PSO-optimized (alpha,beta,rho) produces BETTER results\n"
                      << "     than fixed (alpha,beta,rho). PSO dynamically tunes parameters per\n"
                      << "     source-destination pair, yielding " << std::setprecision(1) << improvement
                      << "% lower average path cost. **\n";
        } else if (fixed_wins > pso_wins) {
            std::cout << "  ** CONCLUSION: Fixed parameters outperformed PSO in this run. **\n";
        } else {
            std::cout << "  ** CONCLUSION: Results are comparable between PSO and fixed. **\n";
        }

        // Also save to file
        std::ofstream comp("pso_vs_fixed_comparison.txt");
        comp << "================================================================\n"
             << "  PSO-OPTIMIZED vs FIXED PARAMETER COMPARISON REPORT\n"
             << "================================================================\n\n"
             << "  PSO approach (old_flowchart.png):\n"
             << "    Phase 1: PSO tunes alpha, beta, rho per S-D pair\n"
             << "    Phase 2: ACO with tuned params\n"
             << "    Phase 3: Dynamic re-planning with tuned params\n\n"
             << "  Fixed approach (latest_flowchart.png):\n"
             << "    Phase 1: ACO with fixed alpha=" << cfg.fixed_alpha
             << ", beta=" << cfg.fixed_beta << ", rho=" << cfg.fixed_rho << "\n"
             << "    Phase 2: Dynamic re-planning with same fixed params\n\n";

        comp << std::setw(5) << "Pair" << std::setw(5) << "Src" << std::setw(5) << "Dst"
             << std::setw(14) << "PSO Cost" << std::setw(14) << "Fixed Cost"
             << std::setw(12) << "Diff %" << std::setw(10) << "Winner" << "\n";
        comp << std::string(65, '-') << "\n";
        ...
        for (int i = 0; i < num_compare; ++i) {
            ...
            double diff_pct = 0;
            if (pr.reached == "YES" && fr.reached) {
                if (pr.final_cost < fr.final_cost - 0.01) winner = "PSO";
                else if (fr.final_cost < pr.final_cost - 0.01) winner = "FIXED";
                else winner = "TIE";
                if (pr.final_cost > 0) diff_pct = (fr.final_cost - pr.final_cost) / pr.final_cost * 100;
            }
            comp << std::setw(5) << i << std::setw(5) << pr.source << std::setw(5) << pr.destination
                 << std::setw(14) << std::fixed << std::setprecision(2) << pr.final_cost
                 << std::setw(14) << (fr.reached ? fr.final_cost : 0.0)
                 << std::setw(11) << std::setprecision(1) << diff_pct << "%"
                 << std::setw(10) << winner << "\n";
        }
        ...
        comp << std::string(65, '-') << "\n\n";
        comp << "SUMMARY:\n"
             << "  Total pairs compared          : " << compared << "\n"
             << "  PSO-Optimized avg final cost   : " << std::setprecision(2) << avg_pso << "\n"
             << "  Fixed-param avg final cost     : " << avg_fixed << "\n"
             << "  PSO improvement over Fixed     : " << std::setprecision(1) << improvement << "%\n\n"
             << "  PSO wins (lower cost)          : " << pso_wins << "\n"
             << "  Fixed wins (lower cost)        : " << fixed_wins << "\n"
        ...
        if (pso_wins > fixed_wins && avg_pso < avg_fixed) {
            comp << "CONCLUSION:\n"
                 << "  PSO-optimized (alpha,beta,rho) produces BETTER results than fixed\n"
                 << "  (alpha,beta,rho). PSO dynamically tunes parameters per source-destination\n"
                 << "  pair based on network topology, yielding " << std::setprecision(1)
                 << improvement << "% lower average path cost.\n\n"
                 << "  This demonstrates that adaptive parameter tuning via PSO provides\n"
                 << "  meaningful improvement over manually chosen fixed parameters.\n";
        } else {
            comp << "CONCLUSION:\n"
                 << "  In this particular run, results were comparable or fixed params\n"
                 << "  performed well. Consider adjusting fixed param values or network.\n";
        }

        comp.close();
        std::cout << "\n  Comparison report saved -> pso_vs_fixed_comparison.txt\n";
    }

    // -- Final output listing --
    std::cout << "\n  Reroute log (fixed)           -> fixed_reroute_log.txt\n";
    std::cout << "  Results CSV (fixed)           -> fixed_all_pairs_results.csv\n";
    std::cout << "  Network analysis (fixed)      -> fixed_network_analysis.txt\n";
    if (cfg.combined_opt) {
        std::cout << "  Results CSV (fixed combined)  -> fixed_combined_pairs_results.csv\n";
        std::cout << "  Network analysis (fxd comb.)  -> fixed_network_analysis_combined.txt\n";
        std::cout << "  Comparison report (fxd comb.) -> fixed_combined_comparison.txt\n";
    }
    std::cout << "  PSO vs Fixed comparison       -> pso_vs_fixed_comparison.txt\n";

    print_banner("Done. All source-destination pairs processed (Fixed params).");
    return 0;
}
